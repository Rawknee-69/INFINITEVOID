...
${client.emoji.dot} AntiNuke Command - \`${prefix}antinuke\``
        ...
        if(!message.guild.members.me.permissionsIn(message.channel).has(Permissions.FLAGS.VIEW_CHANNEL)) return message.author.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} **|** I don't have \`VIEW_CHANNEL\` Permissions in that channel`)]}).catch(() => {});
        if(!message.guild.members.me.permissionsIn(message.channel).has(Permissions.FLAGS.SEND_MESSAGES)) return message.author.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} **|** I don't have \`SEND_MESSAGES\` permissions in that channel.`)]}).catch(() => {});
        if(!message.guild.members.me.permissionsIn(message.channel).has(Permissions.FLAGS.EMBED_LINKS)) return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} **|** I don't have \`EMBED_LINKS\` permissions in this channel.`)]})
        if(!message.guild.members.me.permissionsIn(message.channel).has(Permissions.FLAGS.USE_EXTERNAL_EMOJIS)) return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} **|** I don't have \`USE_EXTERNAL_EMOJIS\` permissions in this channel.`)]})

        if(cmd.arjunop && !client.config.owner.includes(message.author.id))
        {
