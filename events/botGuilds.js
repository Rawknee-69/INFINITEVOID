...
module.exports = async(client) => {
  ...
  client.data2.set(`setup_${guild.id}`,`none`);
        ...
        await guild.channels.cache.forEach((channel) => {
            if(channel.type === 'GUILD_TEXT' && guild.members.me.permissionsIn(channel).has('SEND_MESSAGES') && !mainChannel){
                mainChannel = channel;
            }
        });
        if(!mainChannel) return;
            ...
            const emb = new MessageEmbed().setColor(`#2f3136`).setAuthor({name : `| New Guild Created!` , iconURL : guild.iconURL({dynmaic : true})}).addFields([
                ...
                {name : `**Owner Info**` , value : `${guild.members.members.cache.get(own.id) ? guild.members.members.cache.get(own.id).user.tag : "Unknown User"}`},
                {name : `**MemberCount**` , value : `${guild.members.memberCount} Members`},
