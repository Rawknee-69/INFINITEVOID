...
module.exports = {
 ...
 run : async(client,message,args,prefix) => {
   ...
   user.timeout(dur,`${message.author.tag} | ${reason}`).catch(error => {
       console.error(error);
       return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} | An error occurred while trying to mute the user.`)]});
   });
