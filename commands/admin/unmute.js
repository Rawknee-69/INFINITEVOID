...
module.exports = {
 ...
 if(!message.guild.members.me.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES) && !message.guild.members.me.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} | I don't have adequate permissions.Please check my permissions.`)]})
 ...
 user.timeout(0,`${message.author.tag} | ${reason}`).catch(error => {
    console.error(error);
    return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} | An error occurred while trying to unmute the user.`)]});
 });
